...
to generate molecular descriptors (a.k.a, features) that are numerical
...
vector representation (a.k.a. \emph{descriptors} or \emph{features})
of chemical structures \cite{todeschini2000}, ranging from counts of
elements to eigenvalues of the Laplacian matrix.  Alternatively, we
can compare molecular graphs directly, via kernel methods. In these
...
kernel should be selected to satisfy Mercer's condition, and this is
...
some of explanation as to why a molecule is predicted as toxic or
non-toxic, for example. The former scenario is common in virtual
screening settings, where we require high degree of accuracy and fast
predictions, but do not really care as to why one molecule is active
...
modeled.  In this case, we require to understand the effect of a
...
distributional in nature (such as linear regression and PLS) though
some algorithmic approaches such as random forests can provide some
degree of insight.
...
misprediction (due to the model not bein applicable to a certain class
of inputs) can have significant financial repurcussions. A variety of
