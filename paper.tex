...
\cite{Johnson:1990ys} that states that structurally similar molecules
will exhibit similar properties, the goal of any modeling approach is
to capture and characterize the correlations between structural
features and the observed biological activities. Simultaneously, such
approaches must also describe the likelihood of error (confidence)
when using such models for decision making.  A variety of approaches
can be employed to assess the error in (or conversely, the reliability
of) a prediction, ranging from statistical approaches (XXX) to more
