...
since the 1950s, publicly available (open source) software implementing many of
...
\caption{A hierarchy of chemical structure representations, increasing
  ...
  different purposes. The choice of representation is guided by the
...
problems. 
% If we oversimplify the nature of chemical structures and
...
% dramatically. 
For the field of cheminformatics to flourish, we need
...
data in maintainable ways, 2.  providing sufficient open standards and
tools allowing applications and data to be used across heterogeneous
platforms, and 3. mining the many chemical property spaces in a time-
and space-efficient way. We will discuss all three goals in the
following sections.
...
science as these become available. Still, the integration of
chemical databases with one another remains a challenge, not only due
...
annotation of the ``protein targets'' a molecule binds to, as  
...
% [JH comment] 
% Is this above sentence really true? I thought there was general standardization on 
% UniProt for proteins, GenBank for genes, and that mappings between the other
% sorts of identifiers to these primary ones were actually well maintained?
% As I understand it, the main problem with target representation is that
% protein/gene targets are often unknown, and then it is necessary to map to
% pathway, cell, tissue, biological process etc. --> representing the maximum
% that is known about the mechanism of action for the molecule -- and THESE
% data types are often not well standardized
% [JH comment]
...
from database to database. Hence, we need to be able to capture
molecular graphs in a risk-bounded machine encoded way. %risk-bounded meaning here not clear
...
protonation states (how the molecule changes when one proton is added
to it) and tautomer states (how the molecule can change when a proton
migrates from one part of the molecule to another), the challenge  %This is repeating something stated earlier
...
\subsection{Situating Molecular Data in Biological Context}
\label{sec:prof-ident}

The preceding section focuses on the choice of representation of
chemical structures. In the quest for the discovery of novel
therapeutic agents, it is essential to study molecules as they are
situated in a biological context, underlying the processes of life, 
as metabolites, cofactors for enzymatic reactions, hormones, signalling
molecules and environmental xenobiotics. This gives rise to the need
to adequately represent and organise chemical data across biological
databases such as those containing pathways, protein information or 
biological samples. Integration and processing of data from such 
disparate domains underlies ``systems'' level biological research.
This gives rise to some additional challenges that standard chemical
structural representation cannot address: firstly, the representation of
\textit{classes of chemical entities} -- since, in many cases, a class of 
compounds behave in a certain context in a biological system rather than
a single molecule with a fully-specified structure -- and for similar
reasons, chemical concepts such as \textit{groups} (parts of molecules); 
secondly, the need to represent \textit{non-structural} groupings of 
compounds of interest, since compounds may bind to the same or similar 
targets or they may have  similar biological functions such as acting as 
an anti-neoplastic -- preventing the development of tumors. With the 
many different biological databases needing to refer to these different
chemical entities in an organised and standardized 
fashion to enable cross-database integration for a whole-systems 
perspective, \textit{ontologies} and other semantic technologies 
are being successfully employed to support annotation, classification and
semantic cross-domain querying of chemical entities within
biological systems. The most widely used ontology for biologically relevant
...
Ontologies are based on logical languages of 
varying levels of expressivity, accompanied by sophisticated reasoning
algorithms. They can be used to construct various sorts of semantic
similarity (e.g. similarity in function, similarity in application) 
which complement traditional structural similarity methods \cite{couto2010}.
An open challenge is integrating graph-based chemical
structure information with the wider logic-based ontology information, which 
will allow combined automated reasoning and querying over the different domains 
encoded in a growing number of interlinked ontologies \cite{hastingsowled2010}. 
Enabling automated inferencing in such heterogenous linked systems brings us closer to
achieving a systems level understanding of bioactive chemistry \cite{Oprea:2007fk}.
...
problem, 
% [JH comment]
% Not sure what is meant here? Surely, the complexity of molecular graph enumeration 
% is upper-bounded exponentially in the number of atoms? Additional complexity
% arises from algorithms to select "good" candidates from the candidates?
% Perhaps what is meant is not that the complexity itself is an open problem,
% but finding methods for enumeration which are more efficient is an open problem?
% Alternatively, if complexity in the CS sense is not what is meant here, maybe it will
% help this audience if a different word is used. 
% [JH comment]
...
interest. The difficulty with metabolic network design is that in
...
catalyses (in other words, enzymes must be present to reduce the
energy required for some reactions to take place). That additional
task requires encompassing both chemical structure and protein
sequence information and the development of tools that are at the
...
an example of an extension of statistical QSAR methods to simultaneous
...
into account and, as far as possible, that the receptor also be taken
into account simultaneously. Multi-conformer modeling has been addressed in the
...
% [JH comment]
% we should standardize our nomenclature for the phrase 'Open Source' in the
% document. Here it is capitalized, earlier it has been uncapitalized 
% ('open source'), and below it is hyphenated! (open-source)
% [JH comment]
...
tools simply because this ensures that any interested reader will be able to explore
...
only derives from the chemical graph, but typically also from the 3D
...
years, many chiral molecules or non-standard bonding cases such as 
organometallic compounds cannot be satisfactorily handled by
...
the tautomeric phenomena molecules undergo in practice.
...
identifying lead-like compounds and improving the properties of drug
candidates. In contrast, much bioinformatics software is employed in
upstream areas such as target identification (identifying the protein
that a small molecule binds to).  While certainly
important, this type of research is perhaps not as directly related to
possible profits as an actual small molecule.

Furthermore, chemical data (structure, activity information etc.) has
been harder to acquire and when done on large scales can be extremely
intensive in terms of time and effort, whereas bioinformatics data (sequences for
